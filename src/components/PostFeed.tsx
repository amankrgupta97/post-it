        ...
        if(index===posts.length-1){
          return(
            <li key={post.id} ref={ref}>
              <Post commentAmt={post.comments.length} post={post} subpostitName={post.subpostit.name}/>
            </li>
          )
        }else{
          return <Post commentAmt={post.comments.length} post={post} subpostitName={post.subpostit.name} key={post.id}/>
        }
      })}
    </ul>
  )
